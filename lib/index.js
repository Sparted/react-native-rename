...
readFile(_path2.default.join(__dirname, 'android/app/src/main/res/values/strings.xml')).then(function (data) {
  ...
  _commander2.default.version('2.4.0').arguments('<newName>').option('-b, --bundleID [value]', 'Set custom bundle identifier eg. "com.junedomingo.travelapp"').action(function (newName) {
    ...
    if (bundleID) {
      ...
      return console.log('"' + newName + '" is not a valid name for a project. Please use a valid identifier name (alphanumeric and space) or specify a custom bundleId');
    }
